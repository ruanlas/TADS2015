create database clinica_aula_11_11
go
...
FROM medico RIGHT JOIN ambulatorio 
 ...
 ----
 SELECT ambulatorio_id FROM medico WHERE ambulatorio_id IS NOT NULL
 SELECT DISTINCT ambulatorio_id FROM medico WHERE ambulatorio_id IS NOT NULL

 select * from medico where idade in (28, 41, 45)
 select * from medico where idade > any(SELECT DISTINCT idade FROM medico) 
 ------
