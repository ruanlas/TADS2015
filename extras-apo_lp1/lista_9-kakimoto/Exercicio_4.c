...
void CadastroCarros()
{
    ...
    if (K==1){// se a variável de controle for igual a 1, ele irá executar este bloco
    while (controle != 0){//loop infinito-> ele ficará executando o bloco até que a tecla 0 seja precionada
        fflush(stdin);
        ...
        if (controle!=0){
            K++;
            if (K>20){//caso atinja o limite de 20 cadastros, o loop é encerrado
                break;
            }
        }

    }
}else{//este bloco é executado caso a variavel de controle K seja diferente de 1
    K++;
    while (K<=20){
        fflush(stdin);
        printf("Digite a marca do carro %d: \n", K);
...
void ConsutaTodosElementos()
{
    ...
    fflush(stdin);
    for (i=1; i<=20; i++){
        if (automovel.ano==vetcarros[i].ano && strcmp(automovel.marca, vetcarros[i].marca)==0 && strcmp(automovel.cor, vetcarros[i].cor)==0){ //para ser executado o bloco a seguir, terá que ser satisfeita este condicional
        printf("Preco: R$%.2f\n\n", vetcarros[i].preco);
        }
    }
    printf("Precione 1 para limpar a tela: ");
...
int main()
{
    ...
    op = 9;
       ...
       while (op!=0){//loop infinito -> o loop só é desfeito quando o 0 é precionado
            menuPrincipal();
            scanf("%d", &op);
         switch (op){
            case 1:
